...
public class JwtMockUtil {
    ...
    static String HMAC_SHA256_KEY = "MySecretMySecretMySecretMySecretMySecretMySecretMySecretMySecretMySecretMySecretMySecret";
    ...
    public static boolean validate(String jwtToken) {
        return validate(jwtToken, false);
    }

    public static boolean validate(String jwtToken,boolean isLocalKey) {
        try {
            ...
            if (jws.getAlgorithm().getAlgorithmIdentifier().equals(AlgorithmIdentifiers.HMAC_SHA256)) {
                jws.setKey(Keys.hmacShaKeyFor((HMAC_SHA256_KEY).getBytes()));
    ...
    public static String parseJwt(String jwtToken) {
        ...
        String jwtClaims = null;
        try {
            jwtToken = jwtToken.replace(BEARER, "");
            var jsonWebSignature = new JsonWebSignature();
            jsonWebSignature.setCompactSerialization(jwtToken);
            System.setProperty(sysPropPayloadSkipVerify, "true");//skip verifySignature
            jwtClaims = jsonWebSignature.getPayload();
            log.info("jwt token payload:{}", jwtClaims);
        } catch (JoseException e) {
            log.error("Error occurred while decoding jwt token payload {}.", jwtToken, e);
        }finally {
            System.clearProperty(sysPropPayloadSkipVerify);
        }
        return jwtClaims;
    }
    ...
    public static String mockJwtWithHS256() {
        ...
        try {
            Key key = Keys.hmacShaKeyFor(HMAC_SHA256_KEY.getBytes());
            JwtClaims claims = new JwtClaims();
            claims.setGeneratedJwtId();
            claims.setIssuedAtToNow();
            claims.setSubject("SUBJECT");
    ...
    public static void main(String[] args) throws JoseException {
        var jwtToken = mockJwt(List.of());
        validate(jwtToken,true);

        jwtToken = mockJwtWithHS256();
        validate(jwtToken);
